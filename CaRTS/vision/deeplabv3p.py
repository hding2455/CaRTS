...
class DeepLabv3_plus(VisionBase):
    def __init__(self, params, device):
        super(DeepLabv3_plus, self).__init__(params, device)
        os = params['os']
    ...
    def forward(self, x, return_loss = False):
        ...
        result = self.last_conv(feature_map)
        result = F.upsample(result, size=x['image'].size()[2:], mode='bilinear', align_corners=True)
        if return_loss:
            gt = x['gt']
